...
const validateUser = async (email, password) => {
  const result = await pool.query(
    'select userid, first_name, Last_name, email, password from erp_user where email = $1',
    [email]
  );
...
const generateAuthToken = async (user) => {
  const { userid, email } = user; //todo, need first_name, last_name
  const secret = process.env.secret;
  const token = await jwt.sign({ userid, email }, secret); //todo
  return token;
};
